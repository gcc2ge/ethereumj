...
public class HandshakeHandler extends ByteToMessageDecoder {
    ...
    private ECKey myKey;//my privte key 私钥
    private byte[] nodeId;
    ...
    public void initiate(ChannelHandlerContext ctx) throws Exception {
        ...
        Object msg;
        if (config.eip8()) {//forces peer to send Handshake message in format defined by EIP-8,
        ...
        ctx.writeAndFlush(byteBufMsg).sync();//发送HandShake 消息
    ...
    private void decodeHandshake(final ChannelHandlerContext ctx, ByteBuf buffer) throws Exception {
        ...
        if (handshake.isInitiator()) {
            if (frameCodec == null) {
                ...
                if (frame.getType() == P2pMessageCodes.HELLO.asByte()) {//p2p hello 协议
